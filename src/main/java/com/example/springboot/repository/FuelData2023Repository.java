...
import com.example.springboot.entity.FuelData2023;
...
@Repository
public interface FuelData2023Repository extends JpaRepository<FuelData2023, Long> {
    ...
    @Query(value = "SELECT t.id, t.vendor, t.VehiclePlateNumber, t.TransactionDate, t.UnitPrice, t.Volume, " +
            ...
            "FROM fuel_data_2023 t WHERE STR_TO_DATE(t.TransactionDate, '%c/%e/%Y %h:%i:%s %p') >= :startDate " +
    ...
    @Query(value = """
            ...
            FROM fuel_data_2023 f WHERE VehiclePlateNumber IN(SELECT v.reg_number FROM vehicle_assignment a 
    ...
    @Query(value = """
            SELECT round(SUM(TotalAmount),2) as total_cost FROM fuel_data_2023 f WHERE f.month LIKE '%:monthYear%'
            """, nativeQuery = true)
