...
import com.example.springboot.entity.FuelData2024;
...
@Repository
public interface FuelData2024Repository extends JpaRepository<FuelData2024, Long> {
    ...
    @Query(value = "SELECT t.id, t.vendor, t.VehiclePlateNumber, t.TransactionDate, t.UnitPrice, t.Volume, " +
            ...
            "FROM fuel_data_2024 t WHERE STR_TO_DATE(t.TransactionDate, '%c/%e/%Y %h:%i:%s %p') >= :startDate " +
    ...
    @Query(value = "SELECT round(SUM(t.TotalAmount), 2) " +
            "FROM fuel_data_2024 t " +
    ...
    @Query(value = """
            SELECT SUM(t.Volume) as volume, SUM(t.TotalAmount) as total 
            FROM fuel_data_2024 t 
            WHERE t.VehiclePlateNumber = :vehicle AND t.month LIKE CONCAT('%', :monthYear, '%')
            """, nativeQuery = true)
    List<Map<String, Object>> getDataWithMonth(@Param("vehicle") String vehicle, @Param("monthYear") String monthYear);

    @Query(value = """
            SELECT round(Sum(TotalAmount),2) as total_cost 
            FROM fuel_data_2024 f WHERE VehiclePlateNumber IN(SELECT v.reg_number FROM vehicle_assignment a 
            LEFT JOIN vehicle_details v on v.id=a.vehicle_id WHERE a.branch_id = :branch) AND f.month LIKE CONCAT('%', :monthYear, '%')
            """, nativeQuery = true)
    Map<String, Float> getTotalAmountWithBranch(@Param("monthYear") String monthYear, @Param("branch") String branch);

    @Query(value = """
            SELECT round(SUM(TotalAmount),2) as total_cost FROM fuel_data_2024 f WHERE f.month LIKE CONCAT('%', :monthYear, '%')
            """, nativeQuery = true)
